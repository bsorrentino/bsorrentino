...
**_Thank you_** to all the professionals who have contributed to this white paper by sharing your collective
...
who have vision accessibility needs. Therefore, **_it’s imperative that you use plain language to name
your screens, and that the names include spaces and no abbreviations_**. Also, _we recommend that you
end the name with the word “Screen,” so that the context is understood when the name is announced._
...
- **Data entities derived from the connection:** A Microsoft SharePoint list that’s named **Employees**
    ...
    is Employees. The same PowerApps app can also use **_the same SharePoint connector_** to access
...
discern the _type_ , _purpose,_ and _scope_ of each.
...
**_Be smart!_** PowerApps lets context variables and global variables share the same names. This can cause
...
Then, in the OnVisible property of the **User Profile** screen, call Office365Users.UserProfileV
...
```
o For long single-line formulas, try to insert line breaks in reasonable places: before and
...
- Put the code in one of the controls in the group (the most significant control), and then add
    Select( _controlWithLogic_ ) to the OnSelect property of all the other controls in the
...
timer’s Start property either in the OnVisible property or in the OnSelect property of a hidden
...
_Using views instead of tables_
...
_Using stored procedures through a flow for performance_
**_The biggest performance improvement that you can make for a PowerApps app that uses Microsoft
SQL Server is to call stored procedures from an implementation of Microsoft Flow (that is, a flow)._** This
...
- Use a loading message box or spinner to let the user know that progress is being made behind
    the scenes.
- The Concurrent function is a great way to make your calls to run in parallel. However, when
...
_Use these features at your own risk, and be sure to test your apps thoroughly when you experiment with
these features._
...
**Note:** SaveData and LoadData _work only inside the PowerApps player client app._ Keep this limitation
...
© 201 9 Microsoft Corporation. All rights reserved.
